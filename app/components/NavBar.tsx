...
const NavBar = () => {
  ...
  const borderColor = isLightMode ? "border-p-grey-dark/10" : "border-p-cream/10";
    ...
    <div className="fixed top-6 left-0 right-0 z-50 flex justify-center items-center pointer-events-none gap-3 px-6 transition-all duration-500">
          ...
          >
            ...
            <a href="#">
              <div className="flex items-center gap-2 cursor-pointer select-none">
                <p className={`font-extrabold text-lg tracking-tight transition-colors duration-300 ${textColor}`}>
                  printerQ
                </p>
              </div>
            </a>
            ...
            {/* LINKS */}
            <div className={`hidden md:flex items-center gap-8 font-bold text-sm tracking-wide transition-colors duration-300 opacity-80 ${textColor}`}>
      ...
      <div
        className={`pointer-events-auto rounded-full shadow-2xl transition-all duration-500 ease-in-out cursor-pointer overflow-hidden
          ${showNavButton ? "opacity-100 translate-x-0 w-[140px]" : "opacity-0 translate-x-10 w-0"}
